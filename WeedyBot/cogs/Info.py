...
class Info(commands.Cog):
    ...
    @commands.command(pass_context=True, name="userinfo", aliases=["whois", "whodesu"], brief="Sends information about specified user.")
    ...
    async def emote_detail(self, ctx, emote: discord.Emoji):
        embed = discord.Embed(color=0xf8f8ff, title=f"{emote.name}",
                              timestamp=ctx.message.created_at)
